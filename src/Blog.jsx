...
// Resources / blog. Index lists all posts; BlogPost renders one article with
// internal links to the relevant product page. Styling matches the marketing
// site (spruce/teal on light). Prerendered with Article + FAQ JSON-LD.
      ...
      <main className="wrap blog-article">
        ...
        <div className="blog-more">
          ...
          <div className="blog-more-links">
            {POSTS.filter((o) => o.slug !== p.slug).map((o) => (
...
function BlogNav() {
  return (
    <nav className="blog-topnav">
      <a href="/" className="blog-brand"><b>Med<span>X</span>Flow</b></a>
      <div className="blog-topnav-links">
        <a href="/products">Products</a>
        <a href="/specialties">Specialties</a>
        <a href="/blog" className="on">Resources</a>
        <a href="/#cta" className="blog-nav-cta">Book a demo</a>
      </div>
    </nav>
  );
}
  ...
  return (
    <footer className="blog-foot">
      <div className="wrap">
        ...
        <span><a href="/products">Products</a> · <a href="/specialties">Specialties</a> · <a href="/#cta">Book a demo</a></span>
      </div>
    </footer>
  );
}
...
const BLOG_CSS = `
.blog{--ink:#0D2B52;--spruce:#1A5DAD;--gorse:#17C3B2;--mist:#F2F6FB;--paper:#FFFFFF;--seaglass:#CFE0F2;
  background:var(--paper); color:var(--ink); min-height:100vh; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif; line-height:1.65}
.blog .wrap{max-width:1080px; margin:0 auto; padding:0 24px}
.blog-article{max-width:760px; margin:0 auto; padding:0 24px 80px}
...
.blog h1{font-size:clamp(28px,4.5vw,42px); line-height:1.1; letter-spacing:-.02em; margin:0 0 16px; font-weight:800; text-wrap:balance}
.blog-lede{font-size:18px; color:#3E5064; max-width:620px}
.blog-topnav{display:flex; align-items:center; justify-content:space-between; gap:16px; max-width:1080px; margin:0 auto; padding:18px 24px; flex-wrap:wrap}
.blog-brand{font-size:20px; font-weight:800; color:var(--ink); text-decoration:none; letter-spacing:-.01em}
.blog-brand span{color:var(--gorse)}
.blog-topnav-links{display:flex; align-items:center; gap:22px; font-weight:600; font-size:15px}
.blog-topnav-links a{color:#3E5064; text-decoration:none}
.blog-topnav-links a:hover,.blog-topnav-links a.on{color:var(--spruce)}
.blog-nav-cta{background:var(--ink); color:#fff !important; padding:9px 16px; border-radius:9px}
.blog-head{padding:40px 0 28px; border-bottom:1px solid var(--seaglass); margin-bottom:34px}
...
.blog-card h2{font-size:19px; line-height:1.25; margin:2px 0; font-weight:800; text-wrap:balance}
.blog-card p{font-size:14px; color:#5A6B7E; margin:0}
.blog-meta{font-size:12.5px; color:#8494A6; font-weight:600}
.blog-back{display:inline-block; margin:24px 0 18px; color:var(--spruce); text-decoration:none; font-weight:700; font-size:14px}
.blog-article .blog-cat{margin-bottom:12px}
.blog-article h1{margin-top:6px}
.blog-intro{font-size:19px; line-height:1.6; color:#2B3D50; margin:18px 0 8px; font-weight:500}
.blog-sec{margin-top:30px}
.blog-sec h2{font-size:23px; letter-spacing:-.01em; margin:0 0 12px; font-weight:800; text-wrap:balance}
...
.blog-foot{border-top:1px solid var(--seaglass); margin-top:20px; padding:26px 0; background:var(--mist)}
.blog-foot .wrap{display:flex; justify-content:space-between; gap:16px; flex-wrap:wrap; font-size:13.5px; color:#5A6B7E}
.blog-foot a{color:var(--spruce); text-decoration:none}
@media(max-width:640px){.blog-topnav-links{gap:14px; font-size:14px}}
`;
